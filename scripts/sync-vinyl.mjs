#!/usr/bin/env node
// Regenerates src/lib/vinyl-data.ts from record notes, and fetches missing
// cover art from the Cover Art Archive (via MusicBrainz). Covers already
// present in public/vinyl are never overwritten, so manual overrides are
// preserved.
//
// The local Obsidian vault is the canonical source of truth. Repo-backed inbox
// notes allow cloud sessions to stage new records until they can be imported
// into the vault on a local machine.
//
...
// Source: Obsidian vault Music folder plus repo record inbox. Run \`npm run sync-vinyl\` to refresh.
...
function readGeneratedField(block, field) {
...
async function readGeneratedRecords() {
  ...
  for (const match of raw.matchAll(/  \{\n([\s\S]*?)\n  \},/g)) {
    const block = match[1];
    records.push({
      id: readGeneratedField(block, "id"),
      artist: readGeneratedField(block, "artist"),
      title: readGeneratedField(block, "title"),
      releaseDate: readGeneratedField(block, "releaseDate"),
      coverSrc: readGeneratedField(block, "coverSrc"),
      coverAlt: readGeneratedField(block, "coverAlt"),
...
async function readNoteFiles(dir, label) {
  try {
    ...
    if (err.code === "ENOENT") {
      console.warn(`Skipping missing ${label}: ${dir}`);
      return [];
    }
    throw new Error(`Could not read ${label} ${dir}: ${err.message}`);
  }
}

function ensureUniqueRecord(records, record) {
  ...
  throw new Error(
    `Duplicate record slug "${record.id}" from ${record.source}; already present from ${existing.source}.`,
  );
}
...
async function main() {
  ...
  const vaultNotes = await readNoteFiles(VAULT_DIR, "Obsidian vault Music folder");
  const inboxNotes = await readNoteFiles(INBOX_DIR, "repo record inbox");

  const records = [];
  if (vaultNotes.length > 0) {
    console.log(`Using ${vaultNotes.length} vault note(s) from ${VAULT_DIR}`);
  } else if (inboxNotes.length > 0) {
    const generatedRecords = await readGeneratedRecords();
    console.log(
      `No vault notes found; using ${generatedRecords.length} generated record(s) as the cloud-session baseline.`,
    );
    for (const record of generatedRecords) {
      ensureUniqueRecord(records, record);
    }
  } else {
    console.error(
      `No record notes found. Expected the vault at ${VAULT_DIR} or inbox notes in ${INBOX_DIR}.`,
    );
    process.exit(1);
  }

  const notes = [...vaultNotes, ...inboxNotes];
  if (inboxNotes.length > 0) {
    console.log(`Using ${inboxNotes.length} inbox note(s) from ${INBOX_DIR}`);
  }

  for (const note of notes) {
    const title = note.title;
    const raw = await readFile(path.join(note.dir, note.file), "utf8");
    const fields = parseFrontmatter(raw);
    const artist = fields["Artist"] ?? "Unknown Artist";
    const slug = slugify(title);
    const mbRef = parseMusicBrainzRef(fields["MusicBrainz"]);

    console.log(`• ${title} — ${artist} (${note.label})`);
    ...
    // Covers aren't in the vault/inbox, so only fetch when one is missing.
    // Existing files (auto-fetched or manual overrides) are always kept.
